 ...
 */
package com.btilm305.table.impl;

import com.btilm305.table.ASCIITableHeader;
import com.btilm305.table.spec.IASCIITable;
import com.btilm305.table.spec.IASCIITableAware;
...
public class SimpleASCIITableImpl implements IASCIITable {
	...
	private String getFormattedData(int maxLength, String data, int align) {
		int htmlLength = data.length() - data.replaceAll("\\<[^>]*>", "").length();
		if (data.length() - htmlLength > maxLength) {
			return data;
		}

		boolean toggle = true;

		while ((data.length() - htmlLength) < maxLength) {
			if (align == ALIGN_LEFT) {
				data = data + " ";
	...
	private int getMaxItemLength(List<String> colData) {
		...
		for (int i = 0; i < colData.size(); i++) {
			maxLength = Math.max(colData.get(i).replaceAll("\\<[^>]*>","").length(), maxLength);
		}
		return maxLength;
	}
